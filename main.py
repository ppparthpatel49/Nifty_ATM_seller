...
#
#  Data:     Yahoo Finance (Free, Reliable, Works Everywhere)
...
def wait_for_market_open():
    ...
    if now < market_open:
        ...
        logger.info(f"Waiting {wait_sec/60:.0f} min for market open...")
        print(f"\n⏳ Market opens 9:15 AM IST. Waiting {wait_sec/60:.0f} min...\n")
        time.sleep(wait_sec)
    ...
    try:
        while True:
            try:
                    ...
                    if now.hour >= 15 and now.minute >= 35:
                        if prev_date == now.date() and strategy.daily_trades > 0:
                            telegram.send_daily_summary(
                                strategy.daily_trades, strategy.total_wins,
                                strategy.total_losses, strategy.daily_pnl,
                            )
                            prev_date = None
                        ...
                        logger.info(f"Market closed. Sleeping {sleep_sec/3600:.1f} hrs...")
                ...
                today = now.date()
                if prev_date != today:
                    logger.info("New day. Calculating pivots...")
                    ...
                    if prev_hlc:
                        ...
                        logger.error("Failed to get prev day data!")
                        time.sleep(60)
                        continue
                ...
                if signal and signal != last_signal:
                    if signal == "SELL_PUT":
                        ...
                        telegram.send_exit_signal(
                            ...
                            strike=result["trade_strike"], option_type="PE",
                        ...
                        telegram.send_exit_signal(
                            exit_type="SL", spot=spot,
                            strike=result["trade_strike"], option_type="CE",
                        ...
                        telegram.send_exit_signal(
                            ...
                            strike=result["trade_strike"], option_type=opt_type,
